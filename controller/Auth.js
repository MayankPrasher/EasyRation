...
exports.postAuthsignup = (req, res, next) => {
  ...
  const usertype = "user";
  const password = name; //Work on password
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    console.log(errors.array());
    return res.status(422).render("auth", {
      ...
      if (user) {
        ...
        bcrypt
          ...
          .then((hashedPassword) => {
            ...
            Admin.findOne({
              "aadhar.aadhar_Id": req.body.aadhar,
              "aadhar.name": name,
            }).then((id) => {
              if (id) {
                newUser.save();
                ...
                return res.status(422).render("auth", {
                  errorMessage:
                    "Aadhar or Name is Incorrect, SignUp with correct credentials",
                  oldInput: { aadhar: aadhar, name: name, email: email },
    ...
    Store.findOne({ store_email: store_email, fps_id: entered_fps_id })
      .then((store) => {
        if (store) {
          ...
          const otp = "abc"; //SHOULD BE DYNAMIC
          ...
          return res.status(422).render("store-login",{
            ...
            oldInput:{store_email:store_email,
              entered_fps_id:entered_fps_id
            },
            validationErrors:[]
          })
        }
...
exports.postOtp = (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    ...
    Store.findOne({ otp: entered_otp })
      .then((store) => {
        if (store) {
          const token = generateToken(store.fps_id,'store')
          res.cookie("token",token,{
            httpOnly:true,
            maxAge: 24*60*60*1000,
          })
